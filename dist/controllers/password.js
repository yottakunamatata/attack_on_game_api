...
const sendResetPasswordEmail = (req, res) =>
  __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      const validateCode = Math.floor(
        100000 + Math.random() * 900000,
      ).toString();
      const validationToken = jsonwebtoken_1.default.sign(
      ...
      yield user.save();
      yield (0, help_2.sendEamilValidationCode)(
        to,
        validationToken,
        fronEndUrl,
      );
...
const resetPassword = (req, res) =>
  __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      res
        .status(200)
        .json({ status: true, message: 'Password has be reseted' });
    } catch (err) {
      console.error(err);
      res.status(500).json({ status: false, message: err.message });
    }
